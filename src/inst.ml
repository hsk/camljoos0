...
type instruction =
  ...
  | Ibaload | Ibastore
  | Icaload | Icastore
  | Icheckcast of string
  | Idup     | Idup2
  | Idup2_x1 | Idup2_x2
  | Idup_x1  | Idup_x2
  | Igetfield of string | Igetstatic of string
  | Igoto of label
  | Ii2b | Ii2c | Ii2s
  | Iiadd
  | Iiaload
  ...
  | Iiload of int
  | Iimul | Iineg
  | Iinstanceof of string
  | Iinvokeinterface of msig
  | Iinvokespecial of msig
  | Iinvokestatic of msig
  | Iinvokevirtual of msig
  | Iior  | Iirem
  | Iireturn
  | Iistore of int
  | Iisub | Iixor
  ...
  | Imultianewarray of string * int
  | Inew of string
  | Inop
  | Ipop | Ipop2
  | Iputfield of string | Iputstatic of string
  | Ireturn
  | Isaload | Isastore
  | Iswap
...
let ldc_int_inst i =
    ...
    if      0l <= i && i <=     5l then "iconst_" ^ (Int32.to_string i) else
    if   -128l <= i && i <=   127l then "bipush " ^ (Int32.to_string i) else
    if -32768l <= i && i <= 32767l then "sipush " ^ (Int32.to_string i) else
    "ldc " ^ (Int32.to_string i)
...
let to_asm inst =
  let str =
    ...
    | Iinvokeinterface m -> "invokeinterface " ^ m.msig ^ " " ^
                                   (string_of_int (1 + m.m_nargs))
    | Iinvokespecial m -> "invokespecial " ^ m.msig
    | Iinvokestatic  m -> "invokestatic "  ^ m.msig
    | Iinvokevirtual m -> "invokevirtual " ^ m.msig
    | Iior     -> "ior"
    | Iirem    -> "irem"
    | Iireturn -> "ireturn"
    | Iistore v -> if v <= 3 
...
let stack_change = function
  ...
  | Iinvokeinterface m -> -1 - m.m_nargs + m.m_nreturns
  | Iinvokespecial   m -> -1 - m.m_nargs + m.m_nreturns
  | Iinvokestatic    m ->    - m.m_nargs + m.m_nreturns
  | Iinvokevirtual   m -> -1 - m.m_nargs + m.m_nreturns
